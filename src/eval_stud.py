            ...
            for k in ks:
                ...
                for partition in range(k):
                    ...
                    with torch.no_grad():
                        ...
                        vm= sg.ndata['val_mask']
                        lb = sg.ndata['label']
                        print(partition,(pred[vm] == lb[vm]).float().mean())
                # Compute loss and accuracy
                # val_loss = F.cross_entropy(preds[val_mask].type(torch.FloatTensor), labels[val_mask].type(torch.FloatTensor))
...
# avg_sds = copy.deepcopy(state_dicts)

# # iterate over parameters
# for i,key in enumerate(avg_sds[0]):
#     # for each model, avg with all other models
#     for j,sd in enumerate(state_dicts):
#         for k, osd in enumerate(state_dicts):
#             if k == j: # weight self higher
#                 avg_sds[j][key] *= 0.9
#             avg_sds[j][key] += (osd[key]/len(state_dicts))*.1
...
#                         m = load_model(args.gnn,features,data.num_classes,args.heads, args.dropout)
#                     # m.load_state_dict(checkpoint['model_state_dict'])
#                     m.load_state_dict(avg_sds[partition])
#                     num_params = sum(p.numel() for p in m.parameters() if p.requires_grad)
#                     m.eval()
